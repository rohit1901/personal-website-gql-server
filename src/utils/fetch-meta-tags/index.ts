...
const fetchMetadata = async (
  ...
  for (const [prop, ruleSet] of Object.entries<MetadataRuleSet>(
    metadataRuleSets,
  )) {
    const rule = ruleSet.rules.find(([selector]: [string]) =>
    ...
    if (!metadata[prop]) {
      metadata[prop] =
        ruleSet.defaultValue && makeUrlAbsolute(url, ruleSet.defaultValue);
    }
  }
  return metadata;
};
